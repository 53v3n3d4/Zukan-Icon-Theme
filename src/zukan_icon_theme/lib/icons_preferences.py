...
class ZukanPreference:
    ...
    def _apply_change_icon(p, change_icon):
        if change_icon:
            for k, v in change_icon.items():
                if p['name'] == k:
                    p['preferences']['settings']['icon'] = v

    def _apply_prefer_icon(p, prefer_icon, theme_name, prefer_icon_version):
        if prefer_icon:
    ...
    def _apply_auto_prefer_icon(
        p, theme_name, prefer_icon, auto_prefer_icon, bgcolor, prefer_icon_version
    ):
    ...
    def handle_icon_preferences(
        ...
        """
        # 'change_icon' setting
        ZukanPreference._apply_change_icon(p, change_icon)
        ...
        # 'prefer_icon' setting
        ZukanPreference._apply_prefer_icon(
            p, prefer_icon, theme_name, prefer_icon_version
        )
        ...
        ZukanPreference._apply_auto_prefer_icon(
            p, theme_name, prefer_icon, auto_prefer_icon, bgcolor, prefer_icon_version
        )
        ...
        ZukanPreference._rename_primary_icons(p)
    ...
    def _get_list_icons_preferences(zukan_icons):
        list_all_icons_data = []

        # 'create_custom_icon' setting
        custom_list = [p for p in generate_custom_icon() if 'preferences' in p]
        list_all_icons_data = zukan_icons + custom_list

        return list_all_icons_data

    def _get_file_name(icon_name):
    ...
    def prepare_icon_preference_file(
        ...
        list_all_icons_data = ZukanPreference._get_list_icons_preferences(zukan_icons)

        for p in list_all_icons_data:
            if (
                ...
                filename = ZukanPreference._get_file_name(icon_name)
...
class ZukanPreferenceP2:
    ...
    def _apply_auto_prefer_icon(self, p, prefer_icon_version):
        if self.theme_name not in self.prefer_icon and self.auto_prefer_icon:
            ...
            if self.bgcolor:
                # Prefer light icon
                if self.bgcolor == 'dark' and p['preferences']['settings'][
                    'icon'
                ].endswith('-dark'):
                    prefer_icon_version = p['preferences']['settings']['icon'].replace(
                        '-dark', '-light'
                ...
                # Prefer dark icon
                if self.bgcolor == 'light' and p['preferences']['settings'][
                    'icon'
                ].endswith('-light'):
                    prefer_icon_version = p['preferences']['settings']['icon'].replace(
                        '-light', '-dark'
