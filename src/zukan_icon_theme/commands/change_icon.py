...
# class ChangeIcon:
#     def __init__(self, change_icon_name: str, change_icon_file: str, icon_path: str):
#         self.change_icon_name = change_icon_name
#         self.change_icon_file = change_icon_file
#         self.change_icon, _ = get_change_icon_settings()
#         self.icon_path = icon_path

#     def modify_icon(self):
#         inserted_change_icon = {self.change_icon_name: self.change_icon_file}

#         # Required name and icon input
#         if not self.change_icon_name or not self.change_icon_file:
#             dialog_message = 'Name and icon name inputs are required'
#             sublime.error_message(dialog_message)

#         if self.change_icon_name and self.change_icon_file:
#             if (
#                 self.change_icon_name,
#                 self.change_icon_file,
#             ) not in self.change_icon.items():
#                 self._update_icon_settings(inserted_change_icon)
#                 self._png_exists()

#             elif (
#                 self.change_icon_name,
#                 self.change_icon_file,
#             ) in self.change_icon.items():
#                 dialog_message = '{n} icon already in setting "change_icon"'.format(
#                     n=self.change_icon_name
#                 )
#                 sublime.error_message(dialog_message)

#     def _update_icon_settings(self, inserted_change_icon):
#         self.change_icon.update(inserted_change_icon)
#         set_save_settings(ZUKAN_SETTINGS, 'change_icon', self.change_icon)
...
#         if (
#             not os.path.exists(
#                 os.path.join(self.icon_path, self.change_icon_file + PNG_EXTENSION)
#             )
#             # Primary icons list excluded because 'file_type_image-1' does not
#             # exist in 'icons' folder. It is been renamed to 'file_type_image'
#             and self.change_icon_file not in primary_file_list
#         ):
#             dialog_message = (
#                 '{i} icon PNGs not found.\n\n'
#                 'Insert PNGs in 3 sizes ( 18x16, 36x32, 54x48 ) in {p}'.format(
#                     i=self.change_icon_name, p=self.icon_path
#                 )
#             )
#             sublime.error_message(dialog_message)
...
#     def run(self, edit, change_icon_name: str, change_icon_file: str):
#         handler = ChangeIcon(change_icon_name, change_icon_file, ZUKAN_PKG_ICONS_PATH)
#         handler.modify_icon()
...
# class ChangeIconNameInputHandler(sublime_plugin.TextInputHandler):
#     """
#     Input handler for the icon name.
#     """
...
# class ChangeIconFileInputHandler(sublime_plugin.TextInputHandler):
#     """
#     Input handler for the icon file.
#     """
...
#             else:
#                 if self.icon_name in self.change_icon.keys():
#                     self._reset_single_icon()

#     def _reset_single_icon(self):
#         icon_dict_updated = {
#             k: v for k, v in self.change_icon.items() if k != self.icon_name
#         }
#         set_save_settings(ZUKAN_SETTINGS, 'change_icon', icon_dict_updated)

#         if self.zukan_listener_enabled:
#             logger.info('reseting icon %s', self.icon_name)

#     def _reset_all_icons(self):
#         set_save_settings(ZUKAN_SETTINGS, 'change_icon', {})
...
# class ResetIconCommand(sublime_plugin.TextCommand):
#     """
#     Sublime command to reset icon settings.
#     """

#     def run(self, edit, icon_name: str):
#         handler = ResetIcon(icon_name)
#         handler.reset()
