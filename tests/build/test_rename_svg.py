...
class TestRename:
    ...
    def test_rename_svg(self, a, expected):
        result = zukan_rename_svg.rename_svg(a, DIR_ORIGIN, DIR_DESTINY)
        return result
    ...
    def test_rename_svgs_in_dir(self, a, expected):
        ...
        result = zukan_rename_svg.rename_svgs_in_dir(a, test_dir)
        return result
    ...
    def test_read_dir_oserror(self, caplog):
        ...
        with patch('src.build.helpers.rename_svg.open') as mock_open:
            ...
            zukan_rename_svg.rename_svgs_in_dir('tests/mocks/svg.svg', DIR_DESTINY)
        assert caplog.record_tuples == [
            (
                'src.build.helpers.rename_svg',
                40,
    ...
    def test_not_svg_file(self, capfd):
        zukan_rename_svg.rename_svg('plist.plist', 'tests/mocks/', 'tests/mocks/')
